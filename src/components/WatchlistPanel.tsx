      ...
      {unheld.length === 0 ? (
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
          {unheld.map((c) => (
            <div
              key={c.symbol}
              onClick={() => openChart({ symbol: c.symbol, name: c.name })}
              className="p-3 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between gap-2 cursor-pointer hover:bg-slate-100"
              title="차트 보기"
            >
              <div className="flex items-start gap-2 min-w-0">
                <CompanyLogo symbol={c.symbol} name={c.name} size={26} className="mt-0.5" />
